...
**Does**: decompose the work → propose dispatch plan → pre-confirm (see root CLAUDE.md) → dispatch → aggregate → consolidated report to the principal-report room → append to the learning log (`{{LEARNING_LOG_PATH}}`).

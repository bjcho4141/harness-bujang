...
- `{{BUILD_CMD}}` succeeds
- 0 type errors + record warning count (`{{TYPECHECK_CMD}}`)
- `{{TEST_CMD}}` passes
- E2E (`{{E2E_CMD}}`) — only when 부장 requests

### 2. Regression

- Modified files' **surrounding import paths** still valid
- 0 references to deleted files (grep)
- DB query columns match prod state (`{{DB_TYPES_PATH}}` is the truth)

### 3. Doc sync

- Tracker (`{{TASKS_TRACKER_GLOB}}`) progress percentages recalculated
